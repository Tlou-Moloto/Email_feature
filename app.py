...
app.config['MAIL_PASSWORD'] = 'yttncbjvlsaxqtyd'  #app password after turning the 2 step verifiction on, from your google account
...
def send_booking_confirmation(student_email, student_name, tutor_name, session_time):
    ...
    try:
        ...
        flash(f" Booking successful, but email failed to send: {str(e)}", 'warning')
    ...
    if request.method == 'POST':
        try:
           ...
           booking_date = datetime.strptime(booking_date_str, '%Y-%m-%d').date()
            ...
            flash('Invalid date formate. please use YYYY-MM-DD.', 'error')
            return redirect(url_for('book_session', module_id=module_id))
        ...
        session_time = datetime.combine(booking_date, availability.start_time) # type: ignore
